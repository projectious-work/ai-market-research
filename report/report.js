...
function renderModelAnalytics() {
  ...
  const refName=getRefModel()?.name||'selected reference';
  const rows=data.models.filter(m=>m.tier!=='legacy'&&jurisdictionMatch(m.jurisdiction)).map(m=>({m,...modelScores(m)})).filter(r=>[r.quality,r.speed,r.cost].every(Number.isFinite));
  ...
  const bubble=`<div class="analytics-panel"><h3>Speed × cost efficiency</h3><p>Right and up is better. Bubble area encodes quality vs ${escapeHtml(refName)}; provider color distinguishes the major labs. Numbers map directly to the key below.</p><div class="bubble-readout" aria-live="polite">Hover or focus a bubble or key entry to isolate it.</div><svg class="analytics-svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="Bubble chart comparing speed, cost efficiency and quality versus ${escapeHtml(refName)}">${grid}<line class="axis" x1="${l}" y1="${h-b}" x2="${w-r}" y2="${h-b}"/><line class="axis" x1="${l}" y1="${t}" x2="${l}" y2="${h-b}"/>${bubbles}<text x="${(l+w-r)/2}" y="${h-18}" text-anchor="middle">SPEED SCORE →</text><text transform="translate(22 ${(t+h-b)/2}) rotate(-90)" text-anchor="middle">COST EFFICIENCY →</text></svg>${key}</div>`;
...
function renderDecisionMatrix() {
  ...
  const weights=focusWeights();
  const rows=data.models.filter(m=>m.capability_levels&&m.tier!=='legacy'&&jurisdictionMatch(m.jurisdiction)).map(m=>{
  ...
  }).filter(d=>Number.isFinite(d.cap)&&Number.isFinite(d.market));
  ...
  const refName=getRefModel()?.name||'selected reference';
  panel.innerHTML=`<h3>Capability × market position</h3><p>The vertical axis is the focus-weighted six-category capability score. The horizontal axis averages quality vs ${escapeHtml(refName)}, speed, and cost efficiency. Numbers map directly to the provider-colored key below.</p><div class="bubble-readout" aria-live="polite">Hover or focus a bubble or key entry to isolate it.</div><svg class="analytics-svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="Matrix comparing capability and market scores relative to ${escapeHtml(refName)}">${grid}<line x1="${x(50)}" y1="${t}" x2="${x(50)}" y2="${h-b}" stroke="#c9c6bf" stroke-dasharray="4 4"/><line x1="${l}" y1="${y(50)}" x2="${w-r}" y2="${y(50)}" stroke="#c9c6bf" stroke-dasharray="4 4"/>${marks}<text x="${(l+w-r)/2}" y="${h-22}" text-anchor="middle">MEAN OF QUALITY VS ${escapeHtml(refName.toUpperCase())} + SPEED + COST →</text><text transform="translate(22 ${(t+h-b)/2}) rotate(-90)" text-anchor="middle">6-CATEGORY CAPABILITY SCORE →</text></svg>${key}`;
