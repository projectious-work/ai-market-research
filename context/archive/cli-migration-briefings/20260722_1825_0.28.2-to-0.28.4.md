...
**Status:** pending
**aibox CLI:** v0.28.2 → v0.28.4
**processkit:** v0.27.5

## Summary

aibox has been updated from v0.28.2 to v0.28.4. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.28.2 and v0.28.4 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.28.4 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.28.4` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.28.4"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.28.4.
