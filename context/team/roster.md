...
| `cora` | ai-agent | Cora | `ROLE-product-manager` | senior | Active project interlocutor for the AI Market Research Tool scope. Coordinates the shared implementation harness. |
| `kai` | ai-agent | Kai | `ROLE-software-engineer` | senior | Frontend/backend implementation owner in the shared harness with Cora and Sage. |
| `sage` | ai-agent | Sage | `ROLE-research-scientist` | senior | Research and synthesis owner in the shared harness with Cora and Kai. |
| `atlas` | consultant | Atlas | `ROLE-solutions-architect` | principal | Claude-Code-only architecture consultant. Engaged through written handover documents and returns plans/reviews as written handbacks. |

For the AI Market Research Tool project, the active operating pattern is
four members: Cora, Kai, and Sage work together inside one shared
implementation harness, while Atlas handles architecture in a separate
Claude Code lane through handover documents.
...
This is distinct from a persisted `type=consultant` TeamMember such as
`atlas`, which remains part of the project roster with an explicit
engagement window and scope.
