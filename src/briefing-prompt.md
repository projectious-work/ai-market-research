...
You are running as the **research-scientist** TeamMember (Sage). Your task: refresh `data/market-state.json` with the current state of the AI market across four investigation aspects (models, harnesses, self-hosting, strategy), based on web research.

## Repo layout

- `data/market-state.json` — the canonical JSON state (this is what you edit).
- `data/archive/YYYY-MM-DD.json` — snapshots; the runner archives yesterday's state before invoking you.
- `src/dashboard-context.md` — project profile and tracked dimensions (formerly the prototype's CLAUDE.md).
- `src/sources.md` — canonical URLs to check during research.
- `src/dashboard.template.html` — the HTML scaffold (DO NOT edit). The build script composes `dist/dashboard.html` from template + data.
- `src/scripts/build.sh` — the runner calls this after you finish; you don't.

## Procedure

### Step 1 — Read current state

1. Read `data/market-state.json`. This is yesterday's snapshot.
2. Read `src/dashboard-context.md` for user profile and tracked dimensions.
3. Read `src/sources.md` for canonical URLs to check.
...
1. Write the updated JSON to `data/market-state.json` — that's the only data file you mutate. Validate it parses (e.g. `python3 -m json.tool data/market-state.json > /dev/null`) before exiting.
2. **Do not touch** `src/dashboard.template.html`, `dist/dashboard.html`, or `data/archive/`. The runner script handles archival before invoking you, and runs the build script after.
3. The runner produces the commit. Your job ends after the JSON is valid on disk and you've printed the run summary below.
...
- **You only mutate `data/market-state.json`**. Anything else is out of scope — file an issue or hand off to Kai (software-engineer).
